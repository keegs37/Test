...
def _setup_button_monitroring():  # Backward-compatible misspelling.
    return False
...
def connect_to_makcu():
    ...
    if not ip or not port or not mac:
        ...
        config.kmnet_last_error = "Missing kmNet IP/port/MAC."
        return False
    ...
    try:
        ...
        config.kmnet_last_error = f"kmNet init exception: {e}"
        return False

    if result != 0:
        try:
            result = kmNet.init(ip, int(port), mac)
        except Exception as e:
            print(f"[ERROR] kmNet init failed: {e}")
            config.kmnet_last_error = f"kmNet init exception: {e}"
            return False
        if result != 0:
            ...
            config.kmnet_last_error = f"kmNet init failed with code {result}"
            return False
    ...
    if monitor_port:
        try:
            ...
            config.kmnet_last_error = f"kmNet monitor failed: {e}"
